...
recipename = 'gpi2_fits_keyword_gn_spec.xml'
nfiles_expected = 1
...
def test_spec(pipeline, test_dir):
    ...
    assert stem+"/test_telescop_S20140422S0341_spdc.fits" in outfiles, "Output files didn't contain one of the _podc cubes."
    
    # Are the contents of that file what we expected?
    cube = gpipy.read( stem+"/test_telescop_S20140422S0341_spdc.fits")
