...
recipename = 'gpi2_adi_fits_keyword_project_spec.xml'
file_string = "_keyword_project"
...
def test_spec(pipeline, test_dir):
    ...
    assert stem+"/test_S20140422S0321"+file_string+"_spdc-adim.fits" in outfiles, "Output files didn't contain one of the intermediate ADI images."
    assert stem+"/test_S20140422S0321"+file_string+"_spdc-adim_resadi.fits" in outfiles, "Output files didn't contain the final residual ADI image."    
 
    # Are the contents of that file what we expected?
    cube = gpipy.read( stem+"/test_S20140422S0321"+file_string+"_spdc-adim_resadi.fits" )
